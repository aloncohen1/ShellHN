...
def get_records_metadata(article_metadata: list) -> Tuple[str, str, str]:
    """
...
def calc_rank(article: pd.Series) -> float:
    """
...
def hn_news_api(stories_limit: int = 40) -> pd.DataFrame:
    """
    ...
    hn_df['time_since_sub'] = hn_df.apply(lambda x: (now - x['timestamp']).total_seconds() / 3600,
                                          axis=1)  # calc time since publish
